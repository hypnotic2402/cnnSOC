...
class GCDIO(val w: Int) extends Bundle {
  ...
  val y2 = Input(UInt(w.W))
  val prev = Input(UInt(w.W))
...
trait GCDModule extends HasRegMap {
  ...
  val x2 = Reg(UInt(params.width.W))
  val prev = Reg(UInt(params.width.W))
  val y2 = Wire(new DecoupledIO(UInt(params.width.W)))
  ...
  impl.io.x2 := x2
  impl.io.y2 := y2.bits
  impl.io.input_valid := y2.valid 
  y2.ready := impl.io.input_ready
  ...
  regmap(
    ...
    0x10 -> Seq(
      RegField.w(params.width, x2)),
    0x14 -> Seq(
      RegField.w(params.width, y2)) ) 
...
class WithGCD(useAXI4: Boolean = false, useBlackBox: Boolean = true) extends Config((site, here, up) => {
